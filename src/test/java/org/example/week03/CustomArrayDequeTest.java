...
class CustomArrayDequeTest {
    ...
    @Test
    @DisplayName("Deque should handle capacity expansion correctly")
    void dequeShouldHandleCapacityExpansion() {
        // Add more elements than default capacity
        for (int i = 0; i < 15; i++) {
        ...
        for (int i = 0; i < 15; i++) {
            assertEquals("Element" + i, deque.removeFirst());
        }
        assertTrue(deque.isEmpty());
    }

    @Test
    @DisplayName("Deque should handle capacity expansion with mixed operations")
    void dequeShouldHandleCapacityExpansionWithMixedOperations() {
        ...
        // Add more elements to trigger expansion
        for (int i = 0; i < 10; i++) {
            deque.addLast("New" + i);
        }
    ...
    void constructorWithCollectionShouldInitializeDequeWithElements() {
        Collection<String> coll = Arrays.asList("A", "B", "C");
        CustomArrayDeque<String> newDeque = new CustomArrayDeque<>(coll);
    ...
    void constructorWithEmptyCollectionShouldCreateEmptyDeque() {
        Collection<String> empty = new ArrayList<>();
        CustomArrayDeque<String> newDeque = new CustomArrayDeque<>(empty);
    ...
    void toStringShouldReturnCorrectStringRepresentation() {
        ...
        deque.addFirst("C");
        assertEquals("[C, A, B]", deque.toString());
    }

    @Test
    @DisplayName("ToString should work with null values")
    void toStringShouldWorkWithNullValues() {
        deque.addLast(null);
        assertEquals("[null]", deque.toString());
        deque.addLast("A");
        assertEquals("[null, A]", deque.toString());
    }
}
